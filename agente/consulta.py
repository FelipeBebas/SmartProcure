...
class Campo(str, Enum):
    ...
    desconto_abs     = "desconto_abs"      # regular-oferta em R$      (só se em oferta)
...
CAMPOS_NUMERICOS = {  # texto só em descricao/modelo_codigo
    ...
    Campo.preco_efetivo, Campo.desconto_pct, Campo.desconto_abs, Campo.variacao_pct, Campo.custo_caixa,
...
def preparar_catalogo(itens: list[dict], perfil: PerfilSessao) -> pd.DataFrame:
    ...
    df["desconto_abs"]  = (df["preco_regular"] - df["preco_oferta"]).where(df["em_oferta"])
